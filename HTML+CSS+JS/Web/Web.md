...
| :is(E1,E2,E3)                        | 批量同一样式<br />`:is(E1,E2,E3) a:hover` =`E1`、`E2`、`E3`的`a`鼠标悬浮<br />`:is(ol, ul, menu, dir) :is(ol, ul, menu, dir) :is(ul, menu, dir)`=`ol、ul、ul`下面的`ol, ul, menu, dir`下面的`ul, menu, dir`标签<br />不支持伪元素<br />优先级如果有一个很高，则会带动其他优先级低的 |
...
```js
const obj = { name: '张三' };
Object.prototype.addInfo = function (k, v) {
  /* this即为被.的值 */
  this[k] = v;
  const obj = {};
  obj[k] = v;
...
console.log(obj.addInfo('age', 20)); //{ age: 20 }
